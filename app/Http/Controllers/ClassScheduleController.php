...
class ClassScheduleController extends Controller
{
    ...
    private function visionPayload(string $dataUri, array $days): array
    {
        ...
        $systemPrompt = 'You extract a weekly class timetable from a single photo for a Malaysian polytechnic student app. '
            ...
            . 'A span only continues for as long as the SAME course code is printed in each column it covers. The instant the printed code changes to a different code, or a column has no code printed in it at all, that class has ENDED — start a fresh entry for the new code (or nothing, if the column is blank). Never merge two different course codes into one entry just because their columns sit next to each other with no gap. '
            ...
            . 'Course codes in this kind of table often look almost identical at a glance — e.g. "DFK50083" vs "DFK50093" vs "DFP50463" vs "DFT50194" differ by only one or two characters in the prefix or the digits. Read every character of the code in the cell itself, one at a time, before matching it to the legend — do not assume a code based on a similar-looking one you already used for a different cell. '
            ...
            . 'A cell with no course code printed in it is genuinely empty — never invent a class there because a neighbouring cell, or what "should" logically continue, suggests one. Only output a class for a cell where you can literally see a course code printed in it. '
            . 'Lecturer names are real printed names (often Malay names) — copy the letters exactly as printed. If the name in a cell is too small, blurry or cut off to read with confidence, output null for lecturer rather than write a name that merely sounds plausible; a wrong invented name is worse than a missing one. '
